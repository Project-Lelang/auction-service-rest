...
type Shipment struct {
	...
	BidderAddressId         *int64                 `db:"bidder_address_id"`
	SellerAddressId         *int64                 `db:"seller_address_id"`
	BidderAddressSnapshot   *string                `db:"bidder_address_snapshot"` // JSON
	...
	BidderAddressDeadlineAt data_type.NullDateTime `db:"bidder_address_deadline_at"`
	...
	BidderAddressFailedAt   data_type.NullDateTime `db:"bidder_address_failed_at"`
...
func (s *Shipment) ToMap() map[string]interface{} {
	return map[string]interface{}{
		...
		"user_id":                    s.UserId,
		"bidder_address_id":          s.BidderAddressId,
		"seller_address_id":          s.SellerAddressId,
		"bidder_address_snapshot":    s.BidderAddressSnapshot,
		...
		"bidder_address_deadline_at": s.BidderAddressDeadlineAt,
		...
		"bidder_address_failed_at":   s.BidderAddressFailedAt,
...
// ParseBidderAddressSnapshot deserialises the bidder_address_snapshot JSON column.
func (s *Shipment) ParseBidderAddressSnapshot() *ShipmentAddressSnapshot {
	if s.BidderAddressSnapshot == nil {
		return nil
	}
	var out ShipmentAddressSnapshot
	if err := json.Unmarshal([]byte(*s.BidderAddressSnapshot), &out); err != nil {
		return nil
	}
	return &out
}
